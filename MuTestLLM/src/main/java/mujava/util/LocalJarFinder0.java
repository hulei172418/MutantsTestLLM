...
public class LocalJarFinder0 {
    ...
    public static String buildMavenClasspathFromProject(String projectDir, String mvnCmd)
        ...
        File tmp = File.createTempFile("maven_cp_", ".txt", dir); // 放在项目目录也行
        ...
        int code = p.waitFor();
        if (code != 0) throw new IOException("mvn failed, exit=" + code);

        String cp = new String(java.nio.file.Files.readAllBytes(tmp.toPath()),
    ...
    public static String buildProjectClasspath(String systemHome) throws IOException {
        ...
        String mavenCp;
        try {
            String mvnCmd = MutationSystem.MAVEN_HOME+"/bin/mvn.cmd";
            mavenCp = buildMavenClasspathFromProject(MutationSystem.SYSTEM_HOME, mvnCmd);
        } catch (Exception e) {
    ...
    public static String getJarFilesAsString(String targetDirectory) throws IOException {
        java.util.regex.Pattern EXCLUDE_PATTERN = java.util.regex.Pattern.compile(".*(sources|tests|test-sources|javadoc)\\.jar");
