...
public class MutationSystem extends OJSystem
{
    ...
    public static String MUTANT_HOME_1 = "";
    ...
    public static final String TESTSET_MODE_PROP = "mujava.testset.mode";
    public static final String TESTSET_MODE_EVOSUITE = "evosuite";
    public static final String TESTSET_MODE_ORIGIN = "origin";
    public static final String TESTSET_MODE_LLMS = "llm";

    /** class name without package name that mutation is applied into methodSignature*/
    public static String METHOD_SIGNATURE;

    public static String CLASS_NAME;

    public static String CLASS_NAME_T;

    public static String WHOLE_CLASS_NAME;

    public static List<String> WHOLE_METHOD_NAMES = new ArrayList<>();
...
//                                             "SDL"};
    ...
    public static String[] getAllClassNames(String[] result,String dir)
    {
        ...
        File dirF = new File(dir);
        if (!dirF.exists()) {
            boolean created = dirF.mkdirs();
            if (!created) {
                System.err.println("Failed to create directory: " + dirF.getAbsolutePath());
            }
        }
    ...
    public static void setJMutationStructure() {
        ...
        try {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                if (line.startsWith("MuJava_HOME=")) {
                    sourceHome = line.substring("MuJava_HOME=".length()).trim();
                } else if (line.startsWith("MuJava_RESULT_HOME=")) {
                    resultHome = line.substring("MuJava_RESULT_HOME=".length()).trim();
                } else if (line.startsWith("RESULT_HOME=")) {
                    resultHome = line.substring("RESULT_HOME=".length()).trim();
                }
            }

            if (sourceHome == null || sourceHome.isEmpty()) {
                throw new IOException("MuJava_HOME is missing in mujava.config");
            }

            setJMutationStructure(sourceHome, resultHome);
    ...
    public static void setJMutationStructure(String home_path) {
        setJMutationStructure(home_path, null);
    }

    /** 新接口：显式区分源码模块根与结果模块根 */
    public static void setJMutationStructure(String sourceHome, String resultHome) {
        if (sourceHome == null || sourceHome.trim().isEmpty()) {
            throw new IllegalArgumentException("sourceHome is empty");
        }

        String srcHome = normalizePath(sourceHome);
        String resHome = (resultHome == null || resultHome.trim().isEmpty())
                ? defaultResultHome(srcHome)
                : normalizePath(resultHome);

        SYSTEM_HOME = srcHome;
        SRC_PATH = joinPath(srcHome, "src");
        CLASS_PATH = joinPath(srcHome, "target/classes");
        MUTANT_HOME = joinPath(resHome, "result");

        syncLegacyTestModeProperty();
        TESTSET_PATH = resolveTestSetPath(resHome);
    }

    public static String getTestSetMode() {
        String raw = System.getProperty(TESTSET_MODE_PROP);
        if (raw == null || raw.trim().isEmpty()) {
            if (Boolean.parseBoolean(System.getProperty("mujava.include.evosuite.tests", "false"))) {
                return TESTSET_MODE_EVOSUITE;
            }
            return TESTSET_MODE_ORIGIN;
        }
        return raw.trim();
    }

    public static boolean isOriginTestMode() {
        return TESTSET_MODE_ORIGIN.equalsIgnoreCase(getTestSetMode());
    }

    public static boolean isEvoSuiteTestMode() {
        return TESTSET_MODE_EVOSUITE.equalsIgnoreCase(getTestSetMode());
    }

    private static void syncLegacyTestModeProperty() {
        System.setProperty(
                "mujava.include.evosuite.tests",
                String.valueOf(isEvoSuiteTestMode())
        );
    }

    public static String resolveTestSetPath(String resultHome) {
        String mode = getTestSetMode();
        if (TESTSET_MODE_ORIGIN.equalsIgnoreCase(mode)) {
            return joinPath(resultHome, "origin/target/test-classes");
        }
        return joinPath(resultHome, mode + "/classes");
    }

    private static String normalizePath(String path) {
        return new File(path).getAbsoluteFile().toPath().normalize().toString().replace('\\', '/');
    }

    private static String defaultResultHome(String sourceHome) {
        File src = new File(sourceHome).getAbsoluteFile();
        File parent = src.getParentFile();
        if (parent == null) {
            return src.getPath().replace('\\', '/');
        }
        return parent.getPath().replace('\\', '/');
    }

    private static String joinPath(String base, String child) {
        return new File(base, child).getPath().replace('\\', '/');
    }
}
